...
class DQN_Agent(RL_Base):
    ...
    def evaluate_agent(self, reward_vars):
        ...
        if self.observation_prev_dict['on_off']==0:
            return
        # if reward_vars['load_s']==0:
        #     return
    ...
    def estimate_reward(self, reward_vars):
        ...
        todo NB i shoulkd embedd all the delayed obs, the one that are results of the action I'm evaluating in the rewards'''

        # logger.debug(f"reward vars: {reward_vars}")
        # self.penalty_Dt = -1e-1
        # self.penalty_power = -1e-2
        night_hour = [0,1,2,3,4,5,6]
        self.penalty_Dt = - 0.8
        ...
        # if self.observation_prev_dict['on_off'] == 1 and (tset-t_zone)<0:  #heating
        #     self.penalty_Dt = - 0.01
        # elif self.observation_prev_dict['on_off'] == 1 and self.observation_prev_dict['hour_of_day'] in night_hour:
        #     self.penalty_Dt = - 0.01
        # if self.observation_prev_dict['on_off']==-1 and (tset-t_zone)>0:
        #     self.penalty_Dt = - 0.01
        ...
        # C1 = abs(tset-reward_vars['t_zone']) #if reward_vars['load_s']!=0 else 0.0#/ abs(text-tset)# + beta * abs(reward_vars['t_zone'] - 20.0) # souldh be normnalized over the difference of external temperature and setpoint

        C1 = (tset-reward_vars['t_zone'])**2
        # C1 = (1/81) * (tset-reward_vars['t_zone'])**4
        # C1 = normalize(C1,0, max_diff)

        # C1 = abs(reward_vars['t_zone'] - tset) # temperature component
        if reward_vars['load_s'] > 0:
            C2 = (1/36) *(reward_vars['load_s']* 0.00004)**2
        else:
            C2 = (1/100) *(reward_vars['load_s']* 0.00004)**2



        # try:
        #     C2 = reward_vars['load_s'] / abs(tset-text) if reward_vars['load_s'] > 0 else reward_vars['load_s']/100/abs(tset-text)
        # except ZeroDivisionError:
        #     C2 = reward_vars['load_s']  if reward_vars['load_s'] > 0 else reward_vars['load_s']/100

        # C2 = normalize(reward_vars['load_s'],0,abs(text-tset))
        ...
        self.memory['C2_power'].append(C2)
