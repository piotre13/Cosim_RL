...
class Federate:
    ...
    def register_connections(self):
        ...
        for mod_num in self.connection_ref['end']:
            ...
            for end_info in self.connection_ref['end'][mod_num]:
                ep = self.fed.register_endpoint(end_info['ep_name'])
                if 'subscription' in end_info.keys():
                    for sub in end_info['subscription']:
                        ep.subscribe(sub)
                        logger.debug(f"\n endpoint {ep.name} subscribed to {sub}")


                if str(mod_num) not in self.ends.keys(): #end_info key must be the same of mod_receiver
                    self.ends[mod_num][end_info['ep_name']]=ep

        logger.debug(f"\nFederate registered interfaces\n     Endpoint: {pp.pformat(self.ends)}\n     Publications: {pp.pformat(self.pubs)}\n     Inputs: {pp.pformat(self.inps)}\n")


        logger.debug(f"\nRegistering connections ended for fed {self.fed.name}\n\t list of output vars: {self.out_vars},\n\t list for in_vars{self.in_vars}")
    ...
    def model_instantiation(self):
        ...
        logger.debug(f"\nModels instantiation from fed: {self.fed.name} succesfull!")
        logger.debug(f"\nModel instances names: {self.mod_names}")
